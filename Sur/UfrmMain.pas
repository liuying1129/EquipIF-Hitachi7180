...
var
  INI:tinifile;
  BaudRate,DataBit,StopBit,ParityBit:string;
  autorun:boolean;
begin
  ini:=TINIFILE.Create(ChangeFileExt(Application.ExeName,'.ini'));
...
begin
  ...
  ComDataPacket1Packet(nil,pchar(ls.Text));
  ls.Free;
end;
...
var
  ...
  i,ChkItemNum:integer;
  sYXJB:string;
  CheckDate:string;
begin
  if length(memo1.Lines.Text)>=6000 then memo1.Lines.Clear;//memo只能接受64K个字符
  memo1.Lines.Add(Str);
  ...
  CheckDate:='20'+copy(Str,37,2)+'-'+copy(Str,33,2)+'-'+copy(Str,35,2)+' '+copy(Str,39,2)+':'+copy(Str,41,2);

  IF uppercase(COPY(STR,3,1))='N' THEN sYXJB:='常规' 
  else IF uppercase(COPY(STR,3,1))='Q' THEN sYXJB:='急诊' 
  else sYXJB:='常规';

  //else if uppercase(COPY(STR,3,1))='F' then
  //begin
  //  if copy(Str,5,5)='01001' then SpecNo:='9998'
  //  else if copy(Str,5,5)='02001' then SpecNo:='9997'
  //  else if copy(Str,5,5)='03001' then SpecNo:='9999'
  //  else SpecNo:='9998';
  

  if not trystrtoint(trim(copy(Str,49,3)),ChkItemNum) then//或者48,2
  begin
    memo1.Lines.Add('获取检验项目数量出错!');
    exit;
  end;
  
  ReceiveItemInfo:=VarArrayCreate([0,ChkItemNum-1],varVariant);

  for i :=0  to ChkItemNum-1 do
  begin
    sValue:=trim(copy(Str,52+(i*10)+showIDLen,ItemValuLen));
    showid:=trim(copy(Str,52+(i*10),3));
